    ...
    <header class="fixed inset-x-0 top-0 z-50 bg-borgo/95 backdrop-blur-sm text-white border-b border-white/10">
      <div class="mx-auto max-w-[1320px] px-5 h-14 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-6 text-[14px]">
          ...
          <a href="{{ route('admin.partners.index') }}" class="hover:opacity-100 opacity-90 inline-flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 11a4 4 0 1 1 8 0v2h1a3 3 0 0 1 3 3v3H4v-3a3 3 0 0 1 3-3h1v-2z"/></svg>
            Aliados
          </a>
        ...
        <div class="flex items-center gap-4 text-[14px]">
          <a href="{{ route('home') }}" class="opacity-90 hover:opacity-100 inline-flex items-center gap-2">
              ...
              @foreach($items as $it)
                ...
                <a href="{{ route($it['route']) }}"
                   class="px-3 py-2 rounded-lg block text-sm transition border
                          {{ $active
                              ? 'bg-white border-slate-200 font-medium shadow-sm'
