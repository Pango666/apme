...
@php
  // Desescapar \uXXXX en strings o arrays
  $unesc = function ($v) use (&$unesc) {
      if (is_array($v))  return array_map($unesc, $v);
  ...
  $pairsRaw = \App\Models\Setting::query()->pluck('value','key')->toArray();
  $pairs    = $unesc($pairsRaw);
  ...
  // social puede venir como array o como JSON/string
  $socialVal = $pairs['social'] ?? null;
  if (is_string($socialVal)) {
      $decoded = json_decode($socialVal, true);
      $social = is_array($decoded) ? $unesc($decoded) : ['facebook'=>null,'instagram'=>null,'tiktok'=>null];
  } else {
      $social = is_array($socialVal) ? $unesc($socialVal) : ['facebook'=>null,'instagram'=>null,'tiktok'=>null];
  }
@endphp

{{-- BANNER GLOBAL (newsletter) --}}
@if(session('newsletter_ok') || session('newsletter_err') || ($errors->has('email') && old('email')))
  <div id="flash-banner"
       class="fixed left-0 right-0 z-[100] flex justify-center pointer-events-none">
    <div id="flash-banner-card"
         class="mx-3 my-2 max-w-[720px] w-[calc(100%-1.5rem)]
                pointer-events-auto rounded-lg border shadow-lg px-4 py-3
                @if(session('newsletter_ok')) bg-emerald-50 border-emerald-200 text-emerald-900 @endif
                @if(session('newsletter_err') || ($errors->has('email') && old('email'))) bg-rose-50 border-rose-200 text-rose-900 @endif">
      <div class="flex items-start gap-3">
        ...
        <button type="button" class="ml-auto opacity-70 hover:opacity-100"
                onclick="const r=document.getElementById('flash-banner'); if(r) r.remove();">
  ...
  <script>
    (function () {
      const root = document.getElementById('flash-banner');
      const card = document.getElementById('flash-banner-card');
      if (!root || !card) return;

      // Posicionar justo debajo del topbar + header (sin cortar nada)
      const topbar = document.querySelector('.bg-borgo.text-white.text-sm');
      const header = document.querySelector('header');
      const top = (topbar?.offsetHeight || 0) + (header?.offsetHeight || 0) + 8;
      ...
      <div>
        ...
        <form class="space-y-3" method="POST" action="{{ route('newsletter.subscribe') }}">
          ...
          <input
            ...
            type="email" name="email" value="{{ old('email') }}" placeholder="tu@correo.com" required>
        ...
        {{-- (Opcional) repetir mensaje en footer si quieres --}}
        @if(session('newsletter_ok'))
          <p class="mt-2 text-sm text-emerald-200">{{ session('newsletter_ok') }}</p>
        @endif
        @error('email')
